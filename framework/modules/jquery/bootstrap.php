...
hook_register('html_foot', function () {
	if(JQUERY === 2) {
		$file_url = file_url('framework/modules/jquery/jquery-2.0.2.min.js');
		echo <<<EOS
	<script src="http://code.jquery.com/jquery-latest.min.js"></script>
	<script>window.jQuery || document.write('<script src="${file_url}"><\/script>')</script>
EOS;
	} else {
		$file1_url = file_url('framework/modules/jquery/jquery-1.10.1.min.js');
		$file2_url = file_url('framework/modules/jquery/jquery-migrate-1.2.1.min.js');
		echo <<<EOS
